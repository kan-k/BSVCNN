...
Plot via density `dinvgamma`
```{r}
plot(x=seq(0.001,5, by = 0.002),dinvgamma(x= seq(0.001,5, by = 0.002),5,0.5), ylab = "p(x)", main = "Densities of IG distributions",type='l',col='red',xlab='x')
lines(x=seq(0.001,5, by = 0.002),dinvgamma(x= seq(0.001,5, by = 0.002),3,0.5),col='blue')
legend('topright',legend = c(paste0("IG(5,0.5)")
                             ,paste0("IG(3,0.5)")),lty=c(1),col=c("red",'blue'))
```
...
leglab <- c("method","SGD", "SGLDcorNoise","SGLD_prev noise","Synthetic noise","HSP vs GP-Reg")
line.type <- 'l'
plot(full.res[1,1,],type=line.type,col = 6, ylim = c(2.5, max(full.res[1,,],na.rm = TRUE)),lwd=2,
     main = "Training RMSE",
     ylab = "Batch RMSE",xlab= "Iteration")
