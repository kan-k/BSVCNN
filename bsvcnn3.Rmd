...
lines(x=seq(0.001,5, by = 0.002),dinvgamma(x= seq(0.001,5, by = 0.002),3,0.5),col='blue',lwd=2)
...
plot(hist.train,col=c1,ylim=c(0,600))
...
for(i in 1:50){
  hs.int[i,] <- unlist(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug10_hsvwb_coverage_",i,".csv")))
}
colMeans(hs.int)
```
...
for(i in 1:50){
  hs.int[i,] <- t(as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug10_hsvwb_noscale_",i,".csv"))))
}
...
16,17,23,25,26,30,31,33,36,38,39,42,46,48,5,50,8,
```{r}
#IG with alpha = 5 and initialisation with mean
