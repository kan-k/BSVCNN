...
for( i in setdiff(2:3,failed_run)){  
  res.nn.loss[,which(i==2:3),] <- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn_nm1_loss_jobid_",i,".csv")))
}
...
failed_run <- c(1,4,5,6,7)
lr_vec <- c(10^(-1)*setdiff((1:9),failed_run)) #Note that 1 is not lr = 1, but lr = 1e-4
res.nn.loss <- array(, dim = c(2,9,num.batch*epoch )) 
for( i in lr_vec){
   print(i)
  res.nn.loss[,which(i==lr_vec),] <- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn_adam2_loss_",i,".csv")))
}
res.nn.loss <- res.nn.loss[, 1:length(lr_vec),] #set range
```
...
legend('topright',legend = c('learning_rate',as.character(lr_vec[c(1,2,3)])), pch = 1,col=c("white",1,3,10))
```
Get theta
```{r}
num.batch <- 4
epoch <- 30
#Get the best performance
w.1e10 <- as.matrix(read_feather(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn_adam2_theta_",0.2,".feather")))
...
#Newton
res.nn.loss.nm <- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn_nm1_loss_jobid_",2,".csv")))
#Adam
res.nn.loss.adam <- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn_adam2_loss_",0.1,".csv")))
...
#out-of-sample
plot(full.res[2,1,],type=line.type, ylim = c(min(full.res[2,,]), max(full.res[2,,])),lwd=2,
     main = "Training RMSE",
     ylab = "Batch RMSE",xlab= "Iteration")
