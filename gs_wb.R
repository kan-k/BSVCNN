...
dat_allmat <- fast_read_imgs_mask(list_of_all_images,'/well/nichols/users/qcv214/bsvcnn/pile/HO-combined-fin.nii.gz') #totally equal to dat_allmat
...
for(k in param.grid.part[[JobId]]){

  
#get psi
poly_degree = param_grid[k,1]
a_concentration= param_grid[k,2]
b_smoothness= param_grid[k,3]
...
# psi.mat.nb <- GP.eigen.funcs.fast(nb$maskcoords, poly_degree = poly_degree, a = a_concentration, b = b_smoothness)
#get lambda
...
print("before cbind")
#Get design matrix
print(paste0("dim of bases.nb",dim(bases.nb)))
print(paste0("dim of t(dat_allmat)",dim(t(dat_allmat))))
print(dim(t(bases.nb%*%t(dat_allmat))))
...
# write.csv(out.hs,paste0("/well/nichols/users/qcv214/bnn2/pile/gs_wb",k,".csv"), row.names = FALSE)
write.csv(out.hs,paste0("/well/nichols/users/qcv214/bnn2/pile/gs_wb_rescaled_",k,".csv"), row.names = FALSE)
