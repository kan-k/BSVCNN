...
for(k in c(90,50,25)){
  ...
  write.csv(rbind(age_tab$age[ind.to.use$train],age_tab$age[ind.to.use$test],pred_prior,pred_prior_new), paste0("/well/nichols/users/qcv214/bnn2/pile/pca_lm_pred_usc",k,"_",JobId,".csv"), row.names = FALSE)
  write.csv(c(unlist(t(as.matrix(rsqcal2(pred_prior,pred_prior_new,ind.old = ind.to.use$train,ind.new = ind.to.use$test)))),as.numeric(sub('.*:', '', summary(coefficients(lassofit)[-1]))),sum(abs(coefficients(lassofit)[-1])>1e-5),num.sel.pc),
            paste0("/well/nichols/users/qcv214/bnn2/pile/pca_lm_usc",k,"_",JobId,".csv"), row.names = FALSE)
}
