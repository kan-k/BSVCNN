...
bases.nb.temp <- matrix(,ncol=1,nrow= choose(max(deg_vec)+dimension,dimension))   #######I forgot that it must the same poly degree....
...
# write.csv(rbind(out.hs,out.nm,out.mfvb),paste0("/well/nichols/users/qcv214/bnn2/pile/bsvcnn24feb_gs_",JobId,".csv"), row.names = FALSE)
write.csv(rbind(out.hs,out.nm,out.mfvb),paste0("/well/nichols/users/qcv214/bnn2/pile/bsvcnn25feb_gs_",JobId,".csv"), row.names = FALSE)
