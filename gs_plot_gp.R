...
for(i in sort(mask.reg)){
  gs.opt[which(i==sort(mask.reg)),]<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/pile/gs25feb_ROI_",i,".csv")))
}
...
for(k in mask.reg.part[[JobId]]){
  ...
  poly_degree = param_grid[gs.opt[which(k==sort(mask.reg))],1]
  a_concentration= param_grid[gs.opt[which(k==sort(mask.reg))],2]
  b_smoothness= param_grid[gs.opt[which(k==sort(mask.reg))],3]
  ...
  gp.mask@datatype = 16
  gp.mask@bitpix = 32
  writeNIfTI(gp.mask,paste0('/well/nichols/users/qcv214/bnn2/pile/gs_fin_mask_ROI_',k,'_gp_',poly_degree,a_concentration,b_smoothness))
  print(k)
}
