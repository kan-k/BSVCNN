...
filename <- "jan20_gpnn_olslr_2_1"
prior.var <- 0.05 #was 0.05
learning_rate <- 5e-1 #for slow decay starting less than 1
prior.var.bias <- 1
epoch <- 100
...
while(num.epoch <= epoch){
  ...
  if((num.epoch >=2)& (mean(tail(loss.train,num.batch))>=mean(tail(loss.train,num.batch*2)[1:num.batch]))){
