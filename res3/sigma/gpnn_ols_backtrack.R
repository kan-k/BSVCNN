...
filename <- "jan25_gpnn_ols_backtrack"
...
for(i in 1:n.mask){
  theta.matrix[i,] <- rnorm(n.expan,500,sqrt(prior.var*y.sigma))
...
while(num.epoch <= epoch){
  ...
  for(b in 1:num.batch){
    ...
    map.train <- c(map.train,n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mse(hs_in.pred_SOI,age[mini.batch$train[[b]]]) +n.mask/2*log(y.sigma) +n.mask*n.expan/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) )
    ...
    if(it.num < epoch*num.batch){
      ...
      # theta.martix <- theta.matrix - learning_rate*(theta.matrix/(prior.var*y.sigma) + grad.m*length(train.test.ind$train)) #This is the "grad_x" in jian's backtracking
      #Note that updating weights at the end will be missing the last batch of last epoch
      sum.grad.sq <- c(sum.grad.sq, sum(c((theta.matrix/(prior.var*y.sigma) + grad.m*length(train.test.ind$train)))^2))
      #Update bias
  ...
  if((num.epoch >=2)& (mean(tail(loss.train,num.batch))>=mean(tail(loss.train,num.batch*2)[1:num.batch]) - 0.25*learning_rate*mean(tail(sum.grad.sq,num.batch*2)))){
    theta.matrix<- epoch.theta
...
write_feather(as.data.frame(l.weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/re_',filename,'_lweights_',"_jobid_",JobId,'.feather'))
