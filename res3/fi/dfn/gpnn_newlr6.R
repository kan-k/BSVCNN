...
# 7 Nov, remove regression in the last bit and replace with gradient updates
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    ## updateLast later
    ###Theta
    l.theta <- l.theta*(1-learning_rate*1/(y.sigma)) - learning_rate*l.grad.m* length(train.test.ind$train)
    ###Weight
    l.weights <- partial.gp.centroid  %*% l.theta
    ###Bias
