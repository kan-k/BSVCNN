...
filename <- "dec8_gpnnlin_nhp_v6lr11"
prior.var <- 0.05
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    if(it.num < epoch*num.batch){
      ...
      ## updateLast later
      l.weights <- l.weights - learning_rate*l.grad.m* length(train.test.ind$train)
      ###Bias
      l.bias <- l.bias - learning_rate*c(l.b.grad.m) * length(train.test.ind$train)
