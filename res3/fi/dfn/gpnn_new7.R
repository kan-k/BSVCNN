...
# 8 Nov, remove regression in the last bit and replace with gradient updates
## Add in prior variance for last layer gp

#9 nov correcting sigma
...
prior.var <- 0.05
l.prior.var <- 0.005
learning_rate <- 0.0000005 #for slow decay starting less than 1
prior.var.bias <- 1
...
#data
part_use<- read.csv('/well/nichols/users/qcv214/bnn2/res3/fi/dfn/part_id2.csv')$x
...
train.test.ind <- list()
train.test.ind$test <-  read.csv('/well/nichols/users/qcv214/bnn2/res3/fi/dfn/test_index2.csv')$x
train.test.ind$train <-  read.csv('/well/nichols/users/qcv214/bnn2/res3/fi/dfn/train_index2.csv')$x
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    # Update sigma
    ##this line only works when prior.var is static, use the line further below
    # grad.sigma.m <- mean(length(train.test.ind$train)/(2*y.sigma) - length(train.test.ind$train)/(2*y.sigma^2)*c(grad.loss)^2-1/(2*prior.var*y.sigma^2)*sum(c(theta.matrix)^2)+1/(2*y.sigma)*n.expan*n.mask) -1/(2*l.prior.var*y.sigma^2)*sum(c(l.theta)^2) + 1/(2*y.sigma)*l.expan 
    ##Use this line below
