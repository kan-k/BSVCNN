...
source("/well/nichols/users/qcv214/bnn2/res3/fi/res4_first_layer.R")
source("/well/nichols/users/qcv214/bnn2/res3/fi/res4_second_layer.R")
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    for(i in 1:n.mask){
      temp.mul <- (res3.dat[mini.batch$train[[b]], ]  %*% weights[i,]) + bias[i] #Will yield a  minibatch.size x 1 + bias of that region
