  ...
  for(b in 1:num.batch){
    ...
    for(i in 1:batch_size){
      for(j in 1:nrow(theta.matrix)){
        grad[i,j,] <- grad.loss[i]*beta_fit$HS[j]*relu.prime(hidden.layer[i,j])*res3.dat[mini.batch$train[[b]][i], ] %*% partial.gp[j,,] 
      }
    }
    #Take batch average
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn2_loss_",learning_rate,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nn2_weights_',learning_rate,'.feather'))
