...
theta.matrix <- matrix(rnorm(n.mask*n.expan),nrow=n.mask, ncol= n.expan)
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    grad.loss <- hs_in.pred_SOI-age[mini.batch$train[[b]]]
    mean.grad.loss <- mean(hs_in.pred_SOI-age[mini.batch$train[[b]]])
    #grad <- mean.grad.loss* beta_fit$HS * relu.prime() *res3.dat[mini.batch$train[[1]], ]
    # 1/500x1/500x1/
    #OR
    #This should be in the same dim as `weights`, so for updating w_ij, we require beta_fit_j *relu.prime(i)*input(i) then take avaerge over batch
    grad <- array(,dim = c(batch_size,dim(weights)))
    for(i in 1:batch_size){
      for(j in 1:nrow(weights)){
        grad[i,j,] <- grad.loss[i]*beta_fit$HS[j]*relu.prime(hidden.layer[i,j])*res3.dat[mini.batch$train[[b]][i], ]
      }
    }
    #Take batch average
    ...
    #Update weights
    weights <- weights - learning_rate*grad.m #Note that updating weights at the end will be missing the last batch of last epoch
    #
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nn_loss_",learning_rate,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nn_weights_',learning_rate,'.feather'))
