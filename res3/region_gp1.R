...
out.hs <- c(unlist(t(as.matrix(rsqcal2(hs_in.pred_SOI,hs_pred_SOI,ind.to.use$train,ind.to.use$test)))),
            ...
            pred_R2[1])
write.csv(out.hs,paste0("/well/nichols/users/qcv214/bnn2/pile/region_gp_11apr_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(beta_fit$HS),paste0("/well/nichols/users/qcv214/bnn2/pile/region_gp_11apr_coef_",JobId,'.feather'))
