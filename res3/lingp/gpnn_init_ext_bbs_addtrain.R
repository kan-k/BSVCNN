...
filename <- "apr30_gpnn_init_ext_bbs_addtrain4k_50" #Was feb27
init.num <- 2
...
# ind.unused <- setdiff(seq(n.dat.ori),)

# n.train <- length(train.test.ind$train)

# train.test.ind$train <-  setdiff(seq(n.dat),c(train.test.ind$test))
...
#Storing inv gamma
conj.alpha <- matrix(, nrow=n.mask,ncol=epoch*12)
conj.beta <- matrix(, nrow=n.mask,ncol=epoch*12)
conj.invgamma <-matrix(, nrow=n.mask,ncol=epoch*12)
