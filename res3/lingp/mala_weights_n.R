...
#This one will use MAP with scaling up likelihood by n
...
step_size <- 5e-1 ##Arbitrary

filename <- "mar18_mala_weights_n_lr5e1"
init.num <- 2
...
grad_log_posterior <- function(weights) {
  ...
  gradient <- c((-weights/(prior.var*y.sigma) + grad.m*n.train)) #####Note here that II am scaling by factor of n here on grad.m.
  # Return the gradient
  return(gradient)
}
