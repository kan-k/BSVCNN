...
# Note that currently this is not incorportating the inverse gamma dist
# This also doesn't have bias

#Check if gradient needs factor of n
...
step_size <- 0.0000005 ##Arbitrary

filename <- "mar17_mala_weights_lr0000005"
init.num <- 2
...
log_posterior <- function(weights) {
  ...
  log_pos <- -(n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mse(hs_in.pred_SOI,age) +n.mask/2*log(y.sigma) +n.mask*p.dat/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq)))
  mean.log_pos<- mean(log_pos)
  return(mean.log_pos)
}
...
grad_log_posterior <- function(weights) {
  ...
  gradient <- c((-weights/(prior.var*y.sigma) + grad.m)) #####Note here that II am not rescaling by factor of n here on grad.m... Actually im not sure if it should be there or not.
  # Return the gradient
  return(gradient)
}
...
accepted_params <- matrix(NA, nrow = num_iterations, ncol = p.dat*n.mask)
...
for (i in 1:num_iterations) {
  ...
  log_posterior_ratio <- log_posterior(weights_proposal) - log_posterior(weights_current)
  
  print(paste0("proportion overlap old/new: ", round(sum(c(weights_current)==c(weights_proposal))/length(c(weights_current)),4)))
  print(paste0("log posterior Current: ", log_posterior(weights_current)))
  print(paste0("log posterior Proposal: ", log_posterior(weights_proposal)))
  ...
  accepted_params[i, ] <- c(weights_current)
  # if (i > burn_in) {
  #   accepted_params[i - burn_in, ] <- c(weights_current)
  # }
  print(paste0("Iteration: ",i," out of ",num_iterations, ", iteration: ",Sys.time() -time.epoch))
  ...
  print(paste0("training loss: ",mse(hs_in.pred_SOI,age)))
...
write.csv(loss.train,paste0("/well/nichols/users/qcv214/bnn2/res3/pile/re_",filename,"_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(rsq.train,paste0("/well/nichols/users/qcv214/bnn2/res3/pile/re_",filename,"_rsq_","_jobid_",JobId,".csv"), row.names = FALSE)
