...
filename <- "may7_mala_weights_sub_adapt1_diffinit"
init.num <- JobId 
...
for (i in 1:num_iterations) {
  ...
  ##Cal log pos
  hidden.layer_proposal <- apply(t(t(res3.dat%*%t(weights_proposal)) + bias), 2, FUN = relu)
  ...
  log.likelihood_proposal <- sum(-(n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mse(hs_in.pred_SOI_proposal,age) +n.mask/2*log(y.sigma) +n.mask*p.dat/2*log(y.sigma)))
  ...
  if (u < accept_ratio) {
    ...
    hidden.layer.test <- apply(t(t(res3.dat.test %*% t(weights_proposal)) + bias), 2, FUN = relu)
