...
filename <- "jan23_gpnn_ridgp_2_04_ext"
prior.var <- 0.05 #was 0.05
learning_rate <- 0.4 #for slow decay starting less than 1
prior.var.bias <- 1
epoch <- 400 #was 500, change back to 100
...
y.sigma <- tail(as.vector(unlist(read.csv(paste0('/well/nichols/users/qcv214/bnn2/res3/pile/re_jan21_gpnn_ridgp_2_04_sigma__jobid_',JobId,'.csv')))),1)
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/re_jan21_gpnn_ridgp_2_04_weights_',"_jobid_",JobId,'.feather')))

#Initialising bias (to 0)
bias<- as.vector(unlist(read.csv(paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/re_jan21_gpnn_ridgp_2_04_bias__jobid_',JobId,'.csv'))))
