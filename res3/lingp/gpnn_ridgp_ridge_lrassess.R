...
#In hidden GP, use ridge instead of normal lm
...
filename <- "feb6_gpnn_ridgpridge_2_lr5"
prior.var <- 0.05 #was 0.05
learning_rate <- 5 #for slow decay starting less than 1
prior.var.bias <- 1
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #####
    write_feather(as.data.frame(hidden.layer),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/re_',filename,'_hneurons_',"_jobid_",JobId,'.feather'))
    write_feather(as.data.frame(hidden.layer %*% partial.gp.centroid),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/re_',filename,'_hneuronsBasis_',"_jobid_",JobId,'.feather'))
  
    #####
    fit.lm <- cv.glmnet(hidden.layer %*% partial.gp.centroid ,age[mini.batch$train[[b]]] , alpha = 0,lambda=NULL)#ridge Regress on the minibatch
    
    l.weights <- coef(fit.lm, s=fit.lm$lambda.min)[-1]
    l.weights[is.na(l.weights)] <- 0
    beta_fit <- data.frame(HS = partial.gp.centroid %*% l.weights)
    
    l.bias <- coef(fit.lm, s=fit.lm$lambda.min)[1]
