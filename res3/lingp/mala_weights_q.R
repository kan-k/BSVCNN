...
step_size <- 1e-12 ##Arbitrary

filename <- "mar20_mala_weights_q_lr1e12"
init.num <- 2
...
grad_log_posterior <- function(weights) {
  ...
  gradient <- c((-weights/(prior.var*y.sigma) + grad.m*n.train)) #####Note here that II am not rescaling by factor of n here on grad.m... Actually im not sure if it should be there or not.
  # Return the gradient
  return(gradient)
}
