...
filename <- "feb19_sgld_bb_ig_bbs_cap_burnin_1e-9"
init.num <- 2
...
record.epoch <- 100 #Record last 100 epochs
beta.bb<- 0.5
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    if(e >= (epoch-record.epoch)){
      ...
      if(e==(epoch-record.epoch)){
        ...
        mean.weights <- (mean.weights*mean.num + weights)/(mean.num+1)
        mean.bias <- (mean.bias*mean.num + bias)/(mean.num+1)
        mean.num <- mean.num + 1
      }
  ...
  if(e >=2){
    ...
    learning_rate <- min(lr.cap,ck.new/phi(e))
    gaus.sd <- ifelse(e<=burnin.epoch,0,sqrt(2*learning_rate))
    print(paste0("Noise SD: ",gaus.sd))
