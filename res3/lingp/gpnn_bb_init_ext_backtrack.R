...
filename <- "feb10_gpnn_bb_ext_backtrack"
init.num <- 2
prior.var <- 0.05 #was 0.05
learning_rate <- 0.9 #for slow decay starting less than 1
prior.var.bias <- 1
epoch <- 500 #was 500, change back to 100

error.it <- 0 #counter for backtracking error
...
sum.grad.sq <-  vector(mode = "numeric") #Sum of squared gradient for backtrack
...
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/re_feb7_gpnn_bb_2_beta5_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/re_feb7_gpnn_bb_2_beta5_init_minweights__jobid_',init.num,'.feather')))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/re_feb7_gpnn_bb_2_beta5_init_minbias__jobid_",init.num,".csv"))$x
...
#Start epoch
while(num.epoch <= epoch){
  
  
  #For Backtracking, I need to fix the minibatch
  if(new_batch){
    #randomise data
  ...
  for(b in 1:num.batch){
    ...
    print(paste0("Epoch: ",num.epoch, ", batch number: ", b))
    ...
    if(it.num < epoch*num.batch){
      ...
      # Update sigma
      grad.sigma.m <- mean(n.train/(2*y.sigma) - n.train/(2*y.sigma^2)*c(grad.loss)^2-1/(2*y.sigma^2)*sum(c(weights/prior.var)^2)+1/(2*y.sigma)*p.dat*n.mask)
      ####Note here of the static equal prior.var
      #Update theta matrix
      weights <- weights*(1-learning_rate*1/(prior.var*y.sigma)) - learning_rate*grad.m * n.train
      
      #Note that updating weights at the end will be missing the last batch of last epoch
      
      #Update bias
      bias <- bias*(1-learning_rate*1/(prior.var.bias)) - learning_rate*c(grad.b.m) * n.train
      
      
      sum.grad.sq <- c(sum.grad.sq, sum(c((weights/(prior.var*y.sigma) + grad.m*n.train))^2) + sum(c(bias/prior.var.bias + grad.b.m*(n.train))^2))
      # Update sigma
  ...
  print(paste0("epoch: ",num.epoch," out of ",epoch, ", time taken for this epoch: ",Sys.time() -time.epoch))
  print(paste0("sigma^2: ",y.sigma))
  
  # if(num.epoch %in% c(epoch/2,epoch)){
  #   salient.mat <- matrix(,nrow=n.train, ncol = p.dat)
  ...
  #   writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/bnn2/res3/viz/',filename,'_sal_',JobId,"_epoch_",num.epoch))
  # }
  
  if((error.it <= 4)&(num.epoch >=2)& (mean(tail(map.train,num.batch))>=mean(tail(map.train,num.batch*2)[1:num.batch]) - 0.25*learning_rate*mean(tail(sum.grad.sq,num.batch*2)[1:num.batch]))){
    weights <- epoch.weights
    bias <- epoch.bias
    it.num <- epoch.it.num
    loss.train<- epoch.loss.train
    map.train<- epoch.map.train 
    rsq.train<- epoch.rsq.train
    map.train<- epoch.map
    loss.val<- epoch.loss.test
    rsq.val<- epoch.rsq.test
    sum.grad.sq <- epoch.sum.grad.sq
    learning_rate <- learning_rate*0.75
    error.it <- error.it +1
    new_batch <- FALSE
  }else{
    num.epoch <- num.epoch +1
    epoch.weights <- weights
    epoch.bias <- bias
    epoch.it.num <- it.num
    print("=====================================================")
    print(paste0("Iteration Number: ", it.num))
    print(paste0("Epoch Iteration Number: ", epoch.it.num))
    print("=====================================================")
    epoch.loss.train <- loss.train
    epoch.map.train <- map.train
    epoch.rsq.train <- rsq.train
    epoch.map <- map.train
    epoch.loss.test <- loss.val
    epoch.rsq.test <- rsq.val
    epoch.sum.grad.sq <- sum.grad.sq
    learning_rate <- lr.init #reset learning rate
    error.it <- 0
    new_batch <- TRUE
  }
}
