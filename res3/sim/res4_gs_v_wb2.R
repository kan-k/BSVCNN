...
#Output /well/nichols/users/qcv214/bnn2/res3/pile/sim_res4_gs_v_wb2_ROI_",k,".csv"), row.names = FALSE)
...
#load age
#data
dat_allmat <- as.matrix(read_feather('/well/nichols/users/qcv214/bnn2/res3/sim/sub_res4_dat.feather'))
#Age
...
for(k in mask.reg.part[[JobId]]){
  ...
  mask.temp<-oro.nifti::readNIfTI(paste0('/well/nichols/users/qcv214/bnn2/res3/roi/sim_res4_mask_ROI_',k))
  ...
  for(i in 1:nrow(param_grid)){
    ...
    tryCatch({
      ...
      train_Z <- z.nb[ind.to.use$train,] ##Some are NA because it's my 4000 subjects don't span the entire 4xxx indices
  ...
  write.csv(opt.pos,paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_res4_gs_v_wb2_ROI_",k,".csv"), row.names = FALSE)
}