...
#After seeing the failed result of 0.043, maybe look at the mean of the whole theta matrix = 0.00122582, we want the added noise to be a lot less than that
#so let added noise have s.d. of 0.00122582/3 so that it does not move the theta too much, =>> 3 s.d. covers 99.9% of normal dist, 2 covers 95% and 1 cover 68% 
#s.d. = 0.00122582/3, var = s.d.^2, lr = sd^2/2 this is tiny => 8.34797e-08
#Try s.d. =  0.00122582, var = s.d.^2, lr = sd^2/2 this is tiny => 7.513173e-07
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #specifying lr
    it.num <- it.num +1
    learning_rate <- 7.513173e-07 #for slow decay starting less than 1
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
#inv gamme param
write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.cv,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j8_nnvwbsgldig_Cv_',"_jobid_",JobId,".csv"), row.names = FALSE)
