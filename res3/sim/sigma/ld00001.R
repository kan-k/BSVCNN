...
filename <- "sep7_sgld00001"
gaus.sd <- 0.00001
learning_rate <- gaus.sd^2/2 #for slow decay starting less than 1
epoch <- 100
...
theta.matrix <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_sep7_nnig_theta_',"_jobid_",10,'.feather')))
...
bias<- as.vector(unlist(read.csv( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_sep7_nnig_bias__jobid_10.csv')))

#Load latest variance
prior.var <- tail(as.vector(unlist(read.csv(paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_sep7_nnig_invgam_',"_jobid_",10,".csv")))),n.mask)
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #Update theta matrix
    theta.matrix <- theta.matrix*(1-learning_rate*1/(prior.var*y.sigma)) - learning_rate*grad.m - gaus.noise
