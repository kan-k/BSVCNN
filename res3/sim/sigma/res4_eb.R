...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #Update Cv
    prior.var <- apply(theta.matrix, 1, var)/y.sigma
    it.num <- it.num +1
    # learning_rate <- 0.01
...
# write.csv(c(prior.var.mat[JobId,2]),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_res4_',filename,'_privar_',"_jobid_",JobId,".csv"), row.names = FALSE)
