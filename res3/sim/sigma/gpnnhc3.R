...
filename <- "sep14_nnhc"
learning_rate <- 0.005 #for slow decay starting less than 1
epoch <- 200
...
#NN parameters
it.num <- 1
learning_rate <- 0.001 #for slow decay starting less than 1
...
nu <- matrix(rinvgamma(n.mask*n.expan,0.5,1),nrow=n.mask,ncol=n.expan)   
#
lambda.mat <- matrix(,nrow=n.mask,ncol=n.expan)
for(m in 1:n.mask){
  lambda.mat[m,] <- rinvgamma(n.expan, 0.5,1/nu[m,])
}
tau.vec <- rinvgamma(n.mask, 0.5,1/xi)
prior.var <- tau.vec*lambda.mat
C2 <- 1/(prior.var) #is a matrix
...
# nu <- matrix(,nrow=n.mask,ncol=n.expan) 
#Storing inv gamma
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #Update theta matrix
    theta.matrix <- theta.matrix <- theta.matrix*(1-learning_rate*1/(prior.var*y.sigma)) - learning_rate*grad.m
    ...
    for(i in 1:n.mask){
      nu[i,] <- rinvgamma(n.expan,1,1+1/lambda.mat[i,])
      ...
      tau.vec[i] <- rinvgamma((length(theta.matrix[i,])+1)/2, 1/xi[i] +1/(2*y.sigma)*sum(theta.matrix[i,]^2/lambda.mat[i,]))
      lambda.mat[i,] <- rinvgamma(n.expan,1, 1/nu[i,] + theta.matrix[i,]^2/(2*tau.vec[i])*y.sigma)
  ...
  # print(paste0("prior variance: ",prior.var))
}
...
#inv gamme param
# write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_',filename,'_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
# write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_',filename,'_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
# write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_',filename,'_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
# write.csv(conj.cv,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_',filename,'_Cv_',"_jobid_",JobId,".csv"), row.names = FALSE)