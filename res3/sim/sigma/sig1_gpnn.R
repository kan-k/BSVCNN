...
#Update sigma in lieu of sigma^2
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    # Update sigma
    y.sig <- sqrt(y.sigma)
    grad.sigma.m <- mean(-1/(y.sig) - 1/(y.sig^3)*c(grad.loss)^2-1/(prior.var*y.sig^3)*sum(c(theta.matrix)^2)+1/(y.sig)*n.expan*n.mask)
    ...
    # Update sigma
    y.sig <- y.sig - learning_rate*(grad.sigma.m)
    y.sigma <- y.sig^2
