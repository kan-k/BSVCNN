...
#lr = 0.01
#from nn results, mean of the whole theta matrix = 0.00122582, so I will add noise with s.d. equal to s.d. of the whole theta matrix is 0.293
#sd of added noise = 0.293
#Run for 800 epochs
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    for(i in 1:n.mask){
      gaus.noise[i,] <- rnorm(n.expan,0,0.293)
    }
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
#inv gamma param
write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.cv,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_j10_longsgldhc2_Cv_',"_jobid_",JobId,".csv"), row.names = FALSE)