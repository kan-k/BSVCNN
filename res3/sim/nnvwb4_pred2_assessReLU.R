...
prior.var.mat <- expand.grid(1:50,prior.var.vec)
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    ##########################################
    # print(hidden.layer)
    write.csv(as.matrix(hidden.layer[1:30,]),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_hidmat_","_jobid_",JobId,"_",it.num,".csv"), row.names = FALSE)
    ##########################################
    #Hidden layer
    z.nb <- cbind(1, hidden.layer %*% partial.gp.centroid)
    ##########################################
    write.csv(as.matrix(z.nb),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_zmat_","_jobid_",JobId,"_",it.num,".csv"), row.names = FALSE)
    write.csv(as.matrix(age[mini.batch$train[[b]]]),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_age_","_jobid_",JobId,"_",it.num,".csv"), row.names = FALSE)
    ##########################################
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
...
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_inpred_',"_jobid_",JobId,'.feather'))
temp.frame <- as.data.frame(rbind(pred.test.ind,pred.test.val))
colnames(temp.frame) <- NULL
colnames(temp.frame) <- 1:ncol(temp.frame)
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug30_nnvwb4_outpred_',"_jobid_",JobId,'.feather'))
