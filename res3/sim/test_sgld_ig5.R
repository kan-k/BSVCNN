...
#Fix LR
#Note that s.d. of the whole theta matrix is 0.293, so variance is around 0.0861, so *lr = 0.043*

## Sanity check if gaussian noise is the one causing the difference
# Delete the subtraction of gaussian noise
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #specifying lr
    it.num <- it.num +1
    learning_rate <- 0.043 #for slow decay starting less than 1
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
#inv gamme param
write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.cv,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/test_j8_nnvwbsgldig_Cv_',"_jobid_",JobId,".csv"), row.names = FALSE)
