...
res4.mask.reg <- sort(setdiff(unique(c(mask_subcor)),0))
...
gs.opt <- matrix(,ncol=2,nrow = length(res4.mask.reg))
for(i in sort(res4.mask.reg)){
  gs.opt[which(i==(res4.mask.reg)),]<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_res4_gs_v_wb2_ROI_",i,".csv")))
}
...
partial.gp <- array(, dim = c(length(res4.mask.reg),p.dat,n.expan))
for(i in res4.mask.reg){
  #Load region mask
  ...
  poly_degree = deg_vec
  a_concentration = param_grid[gs.opt[which(1==(res4.mask.reg)),1],2]
  b_smoothness = param_grid[gs.opt[which(1==(res4.mask.reg)),1],3]
  ...
  for(m in setdiff(res4.mask.reg,1)){
    poly_degree = deg_vec
    a_concentration = param_grid[gs.opt[which(m==(res4.mask.reg)),1],2]
    b_smoothness = param_grid[gs.opt[which(m==(res4.mask.reg)),1],3]
    psi.mat.nb <- GP.eigen.funcs.fast(nb.full$maskcoords, poly_degree = poly_degree, a = a_concentration, b = b_smoothness)
    #get lambda
    lambda.nb <- GP.eigen.value(poly_degree = poly_degree, a = a_concentration, b = b_smoothness, d = dimension)
    #Use Karhunen-Loeve expansion/Mercer's theorem to represent our GP as a combination of gaussian realisation, lambda and psi
  ...
  partial.gp[which(i==res4.mask.reg),,] <- (bases.nb.to.use)
}