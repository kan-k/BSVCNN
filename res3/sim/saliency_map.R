...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    dim(z.nb)
    print(b)
    print(head(mini.batch$train[[b]]))
    print(head(age[mini.batch$train[[b]]]))
    ##¡¡¡
  ...
  #plot
  #for subject number
  if(e %in% c(100,150,175,200)){
    for(o in head(train.test.ind$train)){
      
      gp.mask.hs <- res3.mask
      gp.mask.hs[gp.mask.hs!=0] <- c(beta_fit$HS %*% (relu.prime(bias+weights%*%res3.dat[o,])*weights))
      gp.mask.hs@datatype = 16
      gp.mask.hs@bitpix = 32
      writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/bnn2/res3/sim/viz/saliency_seed_',JobId,'_subject_',o,"_epoch_",e))
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_n30_nnvwb4_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_n30_nnvwb4_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_n30_nnvwb4_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_n30_nnvwb4_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)