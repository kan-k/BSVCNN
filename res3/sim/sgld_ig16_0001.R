...
#Change initial value to run #8 of SGD IG(5,0.5)
#Change to shape = 5, scale = 0.5 
#Change lr to 0.01^2/2 since my injected noise is sd 0.01
...
theta.matrix <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbig_a6_theta_',"_jobid_",7,'.feather')))
...
# Load bias
bias<- as.vector(unlist(read.csv( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbig_a6_bias__jobid_7.csv')))
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #specifying lr
    it.num <- it.num +1
    learning_rate <- 0.001^2/2 #for slow decay starting less than 1
    #Specifying gaussian noise
    for(i in 1:n.mask){
      gaus.noise[i,] <- rnorm(n.expan,0,0.001)
    }
    #
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
...
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_inpred_',"_jobid_",JobId,'.feather'))
temp.frame <- as.data.frame(rbind(pred.test.ind,pred.test.val))
colnames(temp.frame) <- NULL
colnames(temp.frame) <- 1:ncol(temp.frame)
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_outpred_',"_jobid_",JobId,'.feather'))

#inv gamme param
write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.cv,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr2_Cv_',"_jobid_",JobId,".csv"), row.names = FALSE)