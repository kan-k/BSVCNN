...
#Change initial value to run #7 of SGD IG(6,0.5)
#Change to shape = 6, scale = 0.5 
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #specifying lr
    it.num <- it.num +1
    learning_rate <- 0.01^2/2 #for slow decay starting less than 1
    #Specifying gaussian noise
    for(i in 1:n.mask){
      gaus.noise[i,] <- rnorm(n.expan,0,0.01)
    }
    #
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
...
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_inpred_',"_jobid_",JobId,'.feather'))
temp.frame <- as.data.frame(rbind(pred.test.ind,pred.test.val))
colnames(temp.frame) <- NULL
colnames(temp.frame) <- 1:ncol(temp.frame)
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_outpred_',"_jobid_",JobId,'.feather'))

#inv gamme param
write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.cv,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/sim_aug8_nnvwbsgldig_lowlr_Cv_',"_jobid_",JobId,".csv"), row.names = FALSE)