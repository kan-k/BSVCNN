...
#NN parameters
it.num <- 1
learning_rate <- 2*((1e-6)+it.num)^-0.6/2 #for slow decay starting less than 1
gaus.noise <- rnorm(0,sqrt(learning_rate*2))
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    it.num <- it.num +1
    learning_rate <- 2*((1e-6)+it.num)^-0.6/2
    gaus.noise <- rnorm(0,sqrt(learning_rate*2))
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nnsgdlsgld1_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnsgdlsgld1_weights_',"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnsgdlsgld1_theta_',"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnsgdlsgld1_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)

write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnsgdlsgd1_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)