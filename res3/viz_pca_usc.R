...
write.csv(rbind(age_tab$age[train.test.ind$train],age_tab$age[train.test.ind$test],pred_prior,pred_prior_new), paste0("/well/nichols/users/qcv214/bnn2/res3/pile/pca_lm_pred_usc_",JobId,".csv"), row.names = FALSE)
write.csv(c(unlist(t(as.matrix(rsqcal2(pred_prior,pred_prior_new,ind.old = train.test.ind$train,ind.new = train.test.ind$test)))),as.numeric(sub('.*:', '', summary(coefficients(lassofit)[-1]))),sum(abs(coefficients(lassofit)[-1])>1e-5),num.sel.pc),
          paste0("/well/nichols/users/qcv214/bnn2/res3/pile/pca_lm_usc_",JobId,".csv"), row.names = FALSE)
...
writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/bnn2/res3/viz/proj_pca_usc_',JobId))