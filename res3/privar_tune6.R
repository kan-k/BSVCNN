...
prior.var.init.vec <- c(45,55) #working JobId from previous result
prior.var.init.mat <- expand.grid(12:21,prior.var.init.vec)
...
#Get minibatch index 
batch_size <- 500
seed <- prior.var.init.mat[JobId,1]
set.seed(seed)
...
theta.sgd <- as.matrix(read_feather(paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nnb6_theta_",0.6,"_jobid_",prior.var.init.mat[JobId,2],".feather")))
prior_var <- c(apply(theta.sgd, 1, var))
C2 <- 1/(2*prior_var)
...
for(i in 1:n.mask){
  theta.matrix[i,] <- rnorm(n.expan,0,sqrt(prior_var[i]))
}
...
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/bnn2/res3/pile/nnb7_loss_",learning_rate,"_jobid_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnb7_weights_',learning_rate,"_jobid_",JobId,'.feather'))
write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnb7_theta_',learning_rate,"_jobid_",JobId,'.feather'))
write.csv(bias,paste0( '/well/nichols/users/qcv214/bnn2/res3/pile/nnb7_bias_',learning_rate,"_jobid_",JobId,".csv"), row.names = FALSE)
