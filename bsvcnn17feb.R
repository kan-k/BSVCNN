...
mask.reg <- setdiff(unique(c(mask.com)),c(0))
#load preset
...
for(i in sort(mask.reg)){
  ...
  nb.norm<- apply(nb$maskcoords,2,norm.func)
...
write.csv(rbind(out.hs,out.nm,out.mfvb),paste0("/well/nichols/users/qcv214/bsvcnn/pile/bsvcnn17feb_rescale_",poly_degree,a_concentration,b_smoothness,"_",JobId,".csv"), row.names = FALSE)
