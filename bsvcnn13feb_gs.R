...
for(i in mask.reg){
  #poly_degree = param_grid[gs.opt[which(i==(mask.reg))],1] #######I forgot that it must the same poly degree....
  poly_degree = 40
  ...
  bases.nb.temp <- matrix(ncol=1,nrow= choose(poly_degree+dimension,dimension)) #######I forgot that it must the same poly degree....
...
write.csv(rbind(out.hs,out.nm,out.mfvb),paste0("/well/nichols/users/qcv214/bnn2/pile/bsvcnn13feb_gs_",JobId,".csv"), row.names = FALSE)
