...
pca.train.sum <- summary(pca.train)
  ...
  num.sel.pc <- which(pca.train.sum$importance[3,]>.90)[1]
  ...
  #lm
  colnames(pca.train.sc) <- NULL
  print(colnames(pca.train.sc))
  ...
  print(length(res_lm))
  #HS
  lassofit2 <- fast_horseshoe_lm(X = cbind(1,as.matrix(pca.train.sc )) ,y = age_tab$age[ind.to.use$train],burnin = 5000)
  pred_prior2<-predict_fast_lm(lassofit2, cbind(1,as.matrix(pca.train.sc)))$median
  pred_prior_new2<-predict_fast_lm(lassofit2, cbind(1,as.matrix(pca.test.sc)))$median
  ...
  write.csv(all_pred, 
            paste0("/well/nichols/users/qcv214/bnn2/smooth/pca_lm_pred_hs_usc",90,"_",JobId,".csv"), row.names = FALSE)
  print("finish writing 1")
  write.csv(rbind(res_lm,res_hs),paste0("/well/nichols/users/qcv214/bnn2/smooth/pca_lm_hs_usc",90,"_",JobId,".csv"), row.names = FALSE)
